...
export const dicePlugin = (function(dice) {
    ...
    this.create_dice_materials = function(face_labels, size, margin) {
       async function create_text_texture(text, color, back_color, bg_url) {
            ...
            if (bg_url) {
                context.drawImage(bg_url, 0,0, 200,200);
            }
        ...
        for (var i = 0; i < face_labels.length; ++i) {
            create_text_texture(face_labels[i], this.label_color, this.dice_color, this.dice_bg_url).then((texture) => {
                materials.push(new THREE.MeshPhongMaterial($t.copyto(this.material_options, { map: texture })));
            });
    ...
    this.dice_color = '#ff0000';
    this.dice_bg_url = false;
    // this.dice_bg_url = '/items/dices/plasma-ball.png';
    ...
    this.dice_box.prototype.reinit = function(container, dimentions) {
        ...
        this.desk = new THREE.Mesh(new THREE.PlaneGeometry(this.w * 2, this.h * 2, 1, 1),
                new THREE.MeshPhongMaterial({ color: that.desk_color, opacity: that.desk_opacity, transparent: that.desk_transparent }));
